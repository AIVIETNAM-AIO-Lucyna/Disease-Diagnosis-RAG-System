...
CHAPTERS: tuple[Chapter, ...] = (
    ...
    Chapter(
        "XX", "External causes of morbidity", "V00–Y99", _bound("V", 0), _bound("Y", 99)
    ),
    Chapter(
        "XXI",
    ...
    Chapter(
        "XXII", "Codes for special purposes", "U00–U85", _bound("U", 0), _bound("U", 85)
    ),
)
...
def write_markdown(references: list[ConditionReference], path: Path) -> None:
    ...
    lines.append(
        f"- A condition is flagged severe when `severity <= {SEVERE_THRESHOLD}` (1 = most severe)."
    )
    lines.append("")
    ...
    dist_str = " · ".join(
        f"sev {sev} = {severity_dist.get(sev, 0)}" for sev in (1, 2, 3, 4, 5)
    )
    lines.append(f"- Severity distribution: {dist_str}")
    lines.append(
        f"- Severe conditions (severity ≤ {SEVERE_THRESHOLD}): **{len(severe)}**"
    )
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--conditions",
        type=Path,
        default=Path("release_conditions.json"),
        help="Path to release_conditions.json",
    )
    parser.add_argument(
        "--outdir", type=Path, default=Path("."), help="Output directory"
    )
    parser.add_argument(
        "--prefix", default="icd10_audit", help="Output file name prefix"
    )
    parser.add_argument(
        "--expected",
        type=int,
        default=None,
        help="Expected condition count for validation",
    )
    parser.add_argument(
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if args.expected is not None and len(references) != args.expected:
        LOGGER.warning(
            "Expected %d conditions, found %d", args.expected, len(references)
        )
