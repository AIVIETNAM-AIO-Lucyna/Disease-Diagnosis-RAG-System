...
\node[stage, text width=2.4cm] (jira) {\textbf{JIRA}\\Scrum board};
\node[stage, text width=2.4cm, right=of jira] (notion) {\textbf{Notion}\\Source of Truth};
\node[stage, text width=2.4cm, right=of notion] (discord) {\textbf{Discord}\\Thông báo nhóm};
\node[stage, text width=2.4cm, above=of notion] (github) {\textbf{GitHub}\\Code + PR};
\node[future, text width=2.4cm, below=of notion] (prism) {\textbf{Prism/Overleaf}\\Báo cáo};
\draw[twoway] (jira) -- node[above, font=\footnotesize, align=center]{Unito\\2 chiều} (notion);
\draw[flow] (jira) to[bend right=38] node[below, font=\footnotesize, align=center]{Jira Automation\\1 chiều} (discord);
