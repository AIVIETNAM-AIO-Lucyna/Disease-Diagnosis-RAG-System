% =============================================================
% sec11_phase2.tex  --  Muc XI: Ke hoach Phase 2 (Module 2)
% Bao cao Phase 1 -- Disease-Diagnosis-RAG-System
% Trich dan: IEEE numeric (biblatex, style=ieee) qua \cite{...}
% Cac khoa trich dan su dung: nogueira2019bert
% Luu y: giu nguyen Hinh fig:roadmap va Bang tab:phase2 (co ref tab:livefull).
% Luu y noi dung: A4 reranker moi o muc ma nguon cuoi Phase 1, danh gia o Phase 2.
% =============================================================
...
\textbf{Chiến lược trên DDXPlus:} giữ truy xuất làm lõi (retrieval-first), nâng dần theo bậc A1 BM25 $\rightarrow$ A2 k-NN $\rightarrow$ A3 Hybrid-RRF (đã chốt ở Phase 1) $\rightarrow$ A4 + reranker cross-encoder~\cite{nogueira2019bert} (\textit{mã nguồn hoàn thiện cuối Phase 1, đánh giá Phase 2}) $\rightarrow$ A5 sinh câu trả lời (Module 2, Phase 2). Mọi bước được đo trên cùng KB + normalizer để so sánh công bằng với bộ offline.
...
\begin{figure}[H]
    ...
    \resizebox{\textwidth}{!}{%
        \begin{tikzpicture}[node distance=1.0cm]
            ...
            \node[future, text width=3.0cm, right=of f1] (g1) {\textbf{Phase 2 - Module 2}\\A5 Qwen3-8B~\cite{qwen2025qwen3} + FastAPI};
            \draw[flow] (p1) -- (f1);
            \draw[flow] (f1) -- (g1);
        \end{tikzpicture}}
...
\begin{table}[H]
    ...
    \begin{tabular}{p{4.3cm}p{4.0cm}p{3.4cm}}
        ...
        Sinh câu trả lời A5 (\texttt{Qwen3-8B}~\cite{qwen2025qwen3}) + FastAPI                                             & Nguyễn Minh Tú                             & chất lượng câu trả lời            \\
