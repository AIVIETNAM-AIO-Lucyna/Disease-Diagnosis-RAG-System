...
variants = {
    ...
    "keyword_text+description": [
        d["keyword_text"] + " " + d.get("description", "") for d in kb
    ],
}
...
per_dis = {
    name: collections.defaultdict(lambda: dict(h1=0, rr=0.0, n=0)) for name in variants
}
empty_q = 0
...
lines.append(
    f"- Queries: {agg[list(variants)[0]]['n']} patients (skipped empty queries: {empty_q}; total rows read: {len(rows)})"
)
lines.append(
    "- Query text = normalize_symptom_phrase(question_en) of patient EVIDENCES (same normalizer as build_kb)"
)
lines.append("- BM25 Okapi k1=1.5 b=0.75; tie-break deterministic\n")
...
for name, a in agg.items():
    n = a["n"]
    lines.append(
        f"| {name} | {pct(a['h1'],n)} | {pct(a['h3'],n)} | {pct(a['h5'],n)} | {a['rr']/n:.4f} | {n} |"
    )
