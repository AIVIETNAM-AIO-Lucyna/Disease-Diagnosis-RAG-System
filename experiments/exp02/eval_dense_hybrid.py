...
SAMPLE = int(sys.argv[1]) if len(sys.argv) > 1 else 5000  # default subsample (dense is heavier)
SEED = 13
...
code_phrase = {c: normalize_symptom_phrase(m.get("question_en", c)) for c, m in evidences.items()}
base_code = lambda ev: ev.split("_@_")[0]

def patient_phrases(evidences_field):
    try: items = ast.literal_eval(evidences_field)
...
class BM25:
    def __init__(self, texts, k1=1.5, b=0.75):
        self.k1=k1; self.b=b
        toks=[tokenize(t) for t in texts]; self.dl=[len(x) for x in toks]
        self.avgdl=sum(self.dl)/len(self.dl); self.inv=collections.defaultdict(list); df=collections.Counter()
        for i,tk in enumerate(toks):
            c=collections.Counter(tk)
            for term,f in c.items(): self.inv[term].append((i,f)); df[term]+=1
        self.N=len(texts); self.idf={t:math.log((self.N-d+0.5)/(d+0.5)+1) for t,d in df.items()}
    def scores(self,qt):
        sc=[0.0]*self.N
        for t in set(qt):
            if t in self.inv:
                for i,f in self.inv[t]:
                    sc[i]+=self.idf[t]*(f*(self.k1+1))/(f+self.k1*(1-self.b+self.b*self.dl[i]/self.avgdl))
        return sc
...
doc_emb = {wd: norm(np.array(model.encode([embed_text_doc(d,wd) for d in kb]))) for wd in (False,True)}
...
def rrf(rank_lists,k=60):
    ...
    for rl in rank_lists:
        for idx,r in rl.items(): fused[idx]+=1.0/(k+r)
    return fused

rows=list(csv.DictReader(open(PATIENTS,encoding="utf-8")))
if SAMPLE and SAMPLE<len(rows): random.seed(SEED); rows=random.sample(rows,SAMPLE)

configs = ["dense(no-desc)","dense(with-desc)","hybrid-RRF(no-desc)","hybrid-RRF(with-desc)"]
...
for row in rows:
    ...
    if not ph: continue
    q=model.encode([embed_text_query(ph)]); q=norm(np.array(q))
    bm_scores=bm25.scores(tokenize(" ".join(ph))); bm_ranks=ranks_from_scores(bm_scores)
    for wd,tag in ((False,"no-desc"),(True,"with-desc")):
        dsc=(doc_emb[wd]@q[0]).tolist(); dranks=ranks_from_scores(dsc)
        # dense
        c=f"dense({tag})"; rank=dranks[gold]; a=agg[c]; a["n"]+=1
        a["h1"]+=rank==1; a["h3"]+=rank<=3; a["h5"]+=rank<=5; a["rr"]+=1/rank
        # hybrid
        fused=rrf([bm_ranks,dranks]); order=sorted(fused,key=lambda i:(-fused[i],diseases[i]))
        hr=order.index(gold)+1; c=f"hybrid-RRF({tag})"; a=agg[c]; a["n"]+=1
        a["h1"]+=hr==1; a["h3"]+=hr<=3; a["h5"]+=hr<=5; a["rr"]+=1/hr

print(f"n={agg[configs[0]]['n']} (model={MODEL})")
print("| config | Hit@1 | Hit@3 | Hit@5 | MRR |")
print("|---|---|---|---|---|")
for c,a in agg.items():
    n=a['n']; f=lambda x:f"{100*x/n:.2f}%"
