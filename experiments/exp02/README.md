...
- `results/`            — committed result artifacts (BM25 report, per-disease CSV, dense/hybrid summary, **live OpenSearch JSON**).
...
|---------------|---------|---------|
| `EXP02_SAMPLE` | `132448` | Patient sample size (`5000` for smoke test) |
| `EXP02_WORKERS` | `8` | Concurrent OpenSearch searches (`asyncio.to_thread`) |
| `EXP02_PROGRESS_EVERY` | `5000` | Progress log interval during full eval |
| `EXP02_REINGEST` | `false` | Set `true` to re-upsert KB before eval |
| `EXP02_RUN_EVAL` | `true` | Set `false` to skip eval and load saved results |
| `EXP02_RESULTS` | `experiments/exp02/results/live_eval_latest.json` | Save/load path for live metrics |
...
### Live OpenSearch results (full validate, n = 132,448)

Committed in `results/live_eval_latest.json` (2026-06-26). Dense uses production `embed_query()` — not directly comparable to offline dense (legacy query template).

| config | Hit@1 | Hit@3 | Hit@5 | MRR |
|--------|-------|-------|-------|-----|
| BM25 (live OpenSearch) | 98.65% | 99.96% | 99.99% | 0.9931 |
| Dense kNN (live OpenSearch) | 79.54% | 93.60% | 96.75% | 0.8709 |
| Hybrid RRF (live OpenSearch) | 92.30% | 98.85% | 99.54% | 0.9565 |

Offline baselines (same n): BM25 98.47% · dense 83.11% (legacy embed) · hybrid 91.17% — see `results/EXP02_eval_report.md` and `results/dense_hybrid_summary.md`.
...
- EXP-02 baselines + live JSON: `experiments/exp02/results/`
...
- Hybrid: RRF k=60 (local fusion of BM25 + dense ranks offline; OpenSearch `hybrid-rrf` pipeline live)
- Query text excludes the disease name (avoid label leakage)
- SAMPLE arg = number of patients (default 5000 in offline scripts; 132448 = full). Sampling random, SEED=13 (reproducible).
...
## Headline results (full, n = 132,448) — offline
